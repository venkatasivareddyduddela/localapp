...
<h1>Funny Shirts Collection</h1>

<!-- Upload form -->
<form action="upload_fshirts.php" method="POST" enctype="multipart/form-data">
...
async function loadShirts() {
  const res = await fetch('fetch_fshirts.php');
  ...
  shirts.forEach(shirt => {
    ...
    img.onclick = async () => {
      if (confirm("Delete Shirt ID: " + shirt.id + " ?")) {
        const res = await fetch("delete_fshirts.php", {
          method: "POST",
