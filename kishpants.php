...
<h1>kishore Pants Collection</h1>

<!-- Upload form -->
<form action="upload_kpants.php" method="POST" enctype="multipart/form-data">
...
async function loadPants() {
  const res = await fetch('fetch_kpants.php');
  ...
  pants.forEach(pant => {
    ...
    img.onclick = async () => {
      if (confirm("Delete Pant ID: " + pant.id + " ?")) {
        const res = await fetch("delete_kpants.php", {
          method: "POST",
