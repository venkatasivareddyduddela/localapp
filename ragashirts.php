...
<h1>raga Shirts Collection</h1>

<!-- Upload form -->
<form action="upload_rshirts.php" method="POST" enctype="multipart/form-data">
...
async function loadShirts() {
  const res = await fetch('fetch_rshirts.php');
  ...
  shirts.forEach(shirt => {
    ...
    img.onclick = async () => {
      if (confirm("Delete Shirt ID: " + shirt.id + " ?")) {
        const res = await fetch("delete_rshirts.php", {
          method: "POST",
