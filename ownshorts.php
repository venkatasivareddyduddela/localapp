...
<h1>Funny Shorts Collection</h1>

<!-- Upload form -->
<form action="upload_fshorts.php" method="POST" enctype="multipart/form-data">
...
async function loadShorts() {
  const res = await fetch('fetch_fshorts.php');
  ...
  shorts.forEach(short => {
    ...
    img.onclick = async () => {
      if (confirm("Delete Shorts ID: " + short.id + " ?")) {
        const res = await fetch("delete_fshorts.php", {
          method: "POST",
